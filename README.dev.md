...
- **Finish → diarize only** — when a realtime session ends, the already-transcribed segments are used for instant diarize-only output; the draft transcript is shown immediately
- **Manual high-accuracy refine** — a "高精度转写" button appears in the transcript header for realtime sessions; clicking it triggers the full ASR pipeline (Phase 2) on demand; progress is shown inline and the transcript is replaced when the job completes
