...
### UI 重设计（三列布局）
- [ ] 三列布局重构：左列历史侧栏 + 中列转写卡片 + 右列纪要卡片（可隐藏，收起时中列自动拉宽）
- [ ] 左列历史侧栏：扫描 output/*.json 列出历史记录（名称/日期/时长），新增转写时写入原始文件名到 json metadata；底部固定"上传文件"和"开始录音"两个入口按钮
- [ ] 中列转写区域扩大：顶部音频播放键 + 进度条，主体按时间行展示转写内容（移除现有大面积上传区）
- [ ] 右列纪要卡片：顶部设置按钮（API 配置 / 模板选择 / 重新生成），纪要内容区，保留最近 3 个版本供切换（存储为 {job_id}_summary.json，数组格式）
